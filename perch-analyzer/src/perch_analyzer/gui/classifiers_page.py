...
def classifier_card(classifier) -> rx.Component:
...
def classifiers():
    ...
    if not all_classifiers:
        content = rx.vstack(
            ...
            rx.text("No classifiers found. Train a classifier to see it here.", style={"fontStyle": "italic"}),
            spacing="4",
            align="center",
        )
    else:
        classifier_cards = [classifier_card(classifier) for classifier in all_classifiers]
        content = rx.vstack(
            rx.heading("Trained Classifiers", size="9"),
            rx.vstack(
                *classifier_cards,
